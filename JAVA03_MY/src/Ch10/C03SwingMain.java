...
class C03GUI extends JFrame implements ActionListener, KeyListener {
	...
	//참조변수 - Worker thread가 만들어졌을 때 연결됨
	Thread th1;
	Thread th2;
	...
	public void actionPerformed(ActionEvent e) {
		//스레드 생성 및 종료
		if (e.getSource() == btn1) { //btn1을 누르면 쓰레드 C03Worker1번이 동작하도록!
			System.out.println("btn1..clicked");	//시작
			
			//Worker thread 객체 생성
			C03Worker1 w1 = new C03Worker1(this);	//this == C03GUI클래스로 객체가 생성되었을때의 주소!
			th1 = new Thread(w1); //메인 스레드와 격리
			th1.start();
		}
		if (e.getSource() == btn2) {
			System.out.println("btn2..clicked");	//종료
			if(th1!=null){//연결된 객체가 있다면
				th1.interrupt(); //해당 쓰레드에 interrupt객체를 생성함으로써 예외 생성
				th1=null;
			}
 
		}
		if (e.getSource() == btn3) { //btn2를 누르면 쓰레드 C03Worker2번이 동작하도록!
			System.out.println("btn3..clicked");	//시작
			
			//Worker thread 객체 생성
			C03Worker2 w2 = new C03Worker2(this);
			th2 = new Thread(w2); //2번 멤버에 연결
			th2.start();
 
		}
		if (e.getSource() == btn4) {
			System.out.println("btn4..clicked");	//종료
			if(th2!=null) {	//연결된 객체가 있다면(쓰레드 객체가 연결되어있으면)
				th2.interrupt(); //interrupt객체를 Worker2 쓰레드에 생성함으로써 예외생성
				th2=null;
			}
