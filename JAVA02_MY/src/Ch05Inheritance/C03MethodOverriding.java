...
public class C03MethodOverriding {
	...
	public static void main(String[] args) {
		...
		System.out.println("----------------------------");
		animal = dog; 		//상위클래스형 참조변수 = 하위객체 : 상위클래스형 참조변수로 하위객체를 연결하는 형태(업캐스팅)
					
		//자료형이 다른데도 문제가 X = 자동형변환 by. 업캐스팅
		animal.Sound();
		
		animal = cat; // ★★★상위클래스를 상속받은 후 하위객체가 재정의한 상태라면 재정의한 결과값이 나옴★★★
					//하위 객체에서 추가한 속성/기능에 접근할 수 없지만 -> 접근하려면 다운캐스팅이 필요함(다운 캐스팅 가능여부확인 : instanceof)
																//해당 자료형에 맞게 만들어서 접근
					//하위 객체에서 재정의한 기능에 접근할 수 있다
							    //재정의한 기능 == 상위클래스의 기능을 재정의한(고쳐쓴)
		animal.Sound();
